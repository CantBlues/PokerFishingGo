...
* Multi-core support. Leaf utilize its modules and [leaf/go](https://poker/github/dolotech/leaf/tree/master/go) to make use of CPU resouces at maximum while avoiding varieties of side effects may be caused.
...
A game server implemented with Leaf may include many modules (e.g. [LeafServer](https://poker/github/dolotech/leafserver)) which all share below traits:

* Each module runs inside a separate goroutine
* Modules communicate with one another via a light weight RPC channel([leaf/chanrpc](https://poker/github/dolotech/leaf/tree/master/chanrpc))
...
[LeafServer](https://poker/github/dolotech/leafserver) is a game server developped with Leaf. Let's start with it.

Download the source code of LeafServer：

```
git clone https://poker/github/dolotech/leafserver
```

Download and install leafserver to GOPATH:

```
go get poker/github/dolotech/leaf
```
...
import (
    "poker/github/dolotech/leaf/network"
)
...
import (
    "poker/github/dolotech/leaf/network/json"
)
...
import (
    "poker/github/dolotech/leaf/log"
...
More references are at [leaf/go](https://poker/github/dolotech/leaf/blob/master/go)。
...
More references are at [leaf/timer](https://poker/github/dolotech/leaf/blob/master/timer)。
...
More references are at [leaf/log](https://poker/github/dolotech/leaf/blob/master/log).

### Leaf recordfile

Leaf recordfile is formatted in CSV([Example](https://poker/github/dolotech/leaf/blob/master/recordfile/test.txt)). recordfile is to manage the configuration for game. The usage of recordfile in LeafServer is quite simple:
...
Refer to [leaf/recordfile](https://poker/github/dolotech/leaf/blob/master/recordfile) for more details.

Learn more
----------

More references are at Wiki [https://poker/github/dolotech/leaf/wiki](https://poker/github/dolotech/leaf/wiki)
